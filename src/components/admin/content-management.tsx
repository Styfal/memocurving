...
// Update the CardSet interface so that createdBy is an object.
interface CardSet {
  id: string;
  title: string;
  createdBy: {
  ...
  // Other fields available in Firestore (cards, createdAt, description, lastReviewed, name, reviewCount, updatedAt)
}
...
export default function ContentManagement({ setNotification }: ContentManagementProps) {
  ...
  useEffect(() => {
    ...
    const fetchCardSets = async () => {
      try {
        ...
        const cardSetsList = querySnapshot.docs.map(docSnap => ({
          id: docSnap.id,
          ...docSnap.data(),
        } as CardSet));
    ...
    const fetchTestSets = async () => {
      try {
        ...
        const testSetsList = querySnapshot.docs.map(docSnap => ({
          id: docSnap.id,
          ...docSnap.data(),
        } as TestSet));
  ...
  // Filter cardSets based on the user's email address.
  // Checks if the card's createdBy.email matches the user's email.
  const getUserCardSets = (userEmail: string) => {
    return cardSets.filter(set => set.createdBy?.email === userEmail);
  };
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-md">
        ...
        <div className="divide-y divide-gray-200">
          {users.map(user => {
            const userCardSets = getUserCardSets(user.email);
                ...
                {isExpanded && (
                  <div className="pl-8 pr-4 pb-4 bg-gray-50 border-t border-gray-100">
                    {userCardSets.length > 0 && (
                      <div className="mb-4">
                        <h3 className="text-md font-medium mb-2 text-gray-700">Cards</h3>
